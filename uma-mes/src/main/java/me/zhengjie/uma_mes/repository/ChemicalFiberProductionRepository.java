...
public interface ChemicalFiberProductionRepository extends JpaRepository<ChemicalFiberProduction, Integer>, JpaSpecificationExecutor<ChemicalFiberProduction> {
   ...
   /* @Query(value = "select *, " +
    ...
    @Query(value = "SELECT * " +
            "FROM (" +
            "SELECT product.*,label.shifts, production.number, production.machine_number,label.print_time as print_time," +
            ...
            "GROUP BY product.id, label.print_time ORDER BY label.print_time" +
