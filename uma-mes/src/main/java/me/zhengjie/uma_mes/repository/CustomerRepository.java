...
public interface CustomerRepository extends JpaRepository<Customer, Integer>, JpaSpecificationExecutor<Customer> {
    @Query(value = "SELECT customer.id,customer.NAME,customer.CODE,customer.address,customer.contacts,customer.contact_phone,customer.remark,customer.create_date,customer.create_user,customer.del_flag,customer.full_name,customer.account,customer.over_arrears," +
            " (SELECT ifnull(sum( balance ),0) FROM uma_chemical_fiber_delivery_note n WHERE  n.customer_id = customer.id and n.create_date < :startTime) as total_arrears" +
            ",(SELECT ifnull(sum( balance ),0) FROM uma_chemical_fiber_delivery_note n WHERE  n.customer_id = customer.id and n.create_date BETWEEN :startTime and :endTime) as current_arrears" +
